...
def svg_to_png(
    svg: str, *, width: int, height: int, backend: Backend | None = None
) -> bytes:
...
def render_png(
    ...
    *,
    ...
    w = int(width) if width is not None else int(round(float(canvas.get("width", 512)) * scale))
    h = int(height) if height is not None else int(round(float(canvas.get("height", 512)) * scale))
    if w <= 0 or h <= 0:
...
class IconBundle:
    ...
    def manifest(self) -> list[dict]:
        return [
            {"name": name, "bytes": len(data)} for name, data in sorted(self.files.items())
        ]
    ...
    if "ico" in formats:
        ...
        with Image.open(io.BytesIO(pngs[largest])) as im:
            ...
            im.convert("RGBA").save(
                buf, format="ICO", sizes=[(s, s) for s in sorted(ico_sizes)]
            )
