...
def _schema_errors(scene: dict) -> list[IsoError]:
    ...
    for e in sorted(_schema_validator().iter_errors(scene), key=lambda e: list(e.absolute_path)):
        ...
        if e.validator == "oneOf" and isinstance(e.instance, dict) and "type" in e.instance:
            ...
            best = next(
                (c for c in e.context or [] if f"'{declared}'" in c.message or declared in str(c.schema_path)),
                None,
            )
            if best is not None:
                errors.append(
            ...
            if not (0 <= x < gw and 0 <= y < gd and 0 <= z < gh):
                errors.append(
                    IsoError(
                        ...
                        f"absolute origin ({x}, {y}, {z}) is outside grid "
                        f"{gw}x{gd}x{gh}",
                        f"{path}.at",
                        shape_id=sid,
                    )
                ...
                for axis, o, s, limit in (
                    ("x", x, sx, gw), ("y", y, sy, gd), ("z", z, sz, gh)
                ):
                    if o + s > limit:
                        errors.append(
                            IsoError(
                                IsoErrorCode.EXTENT_OVERFLOW,
                                f"extent on {axis} reaches {o + s:g}, exceeding grid "
                                f"limit {limit}",
                                f"{path}.size.{axis}",
                                shape_id=sid,
                            )
