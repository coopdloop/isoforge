...
from isoforge_py.isodsl import validate
from isoforge_py.isodsl.color import parse_hex, rgb_to_oklab, shade, to_hex
from isoforge_py.render.svg import render_svg
...
class TestPrimitiveCoverage:
    ...
    def test_cylinder_segment_count_is_honoured(self):
        scene = {
            ...
            "shapes": [
                {"id": "c", "type": "cylinder", "at": {"x": 0, "y": 0, "z": 0},
                 "size": {"x": 2, "y": 2, "z": 2}, "segments": 12, "fill": "@palette.a"}
            ],
        }
        assert validate(scene).valid
        svg = render_svg(scene)
...
class TestColor:
    ...
    def test_positive_offset_lightens(self):
        assert rgb_to_oklab(parse_hex(shade("#7C5CFF", 0.2))).L > rgb_to_oklab(
            parse_hex("#7C5CFF")
        ).L
