...
from isoforge_py.agent.orchestrator import Conversation, Orchestrator, TurnError, apply_patch
from isoforge_py.agent.providers.base import ProviderError
from isoforge_py.agent.tools import PATCH_SCENE, SET_SCENE
...
class TestSetScene:
    ...
    async def test_missing_scene_argument_is_rejected(self, conversation):
        from isoforge_py.agent.providers.base import CompletionResult, ToolCall
...
class TestPatchScene:
    async def test_patch_applies_to_current_scene(self, seeded):
        provider = FakeProvider(
            [patch_scene_result([{"op": "replace", "path": "/shapes/0/fill",
                                  "value": "@palette.accent"}], "recolor")]
        )
    ...
    async def test_append_shape_via_dash_pointer(self, seeded):
        new_shape = {
            "id": "second", "type": "cube", "at": {"x": 1, "y": 1, "z": 0},
            "fill": "@palette.accent",
        }
        provider = FakeProvider(
...
class TestRepairLoop:
    ...
    async def test_repair_prompt_carries_error_codes(self, conversation, single_cube):
        ...
        provider = FakeProvider(
            [set_scene_result(broken), set_scene_result(single_cube)]
        )
        await Orchestrator(provider).run_turn(conversation, "cube")
